...
/* 
 * Modfied by ATN: 
 *	myqsample Seed N FromFile ToFile		OR
 *	myqsample Seed N FromFile Sample Complement
 * NOTE: if Seed==0 then datetime() is used as the seed value
 * 
 * 
 * 
...
main(argc,argv)
	...
	if(argc==5) complement=FALSE;
	else if(argc==6) complement=TRUE;
	else {
	  printf("Command should be one of the following\n\t<qsample Seed N FromFile ToFile>\n\t<qsample Seed N FromFile Sample Complement>\n");
	  exit(0);
	}
	sscanf(argv[1],"%ld",&seedval);
	if(!seedval) seedval = datetime();

	sscanf(argv[2],"%d",&samplesize);
	inname=argv[3]; outname1=argv[4];
